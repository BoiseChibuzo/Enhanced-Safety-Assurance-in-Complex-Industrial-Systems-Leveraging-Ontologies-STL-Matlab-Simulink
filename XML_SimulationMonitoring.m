...
checkAndRespond(Bdata, 'Reactor Pressure Alt0', 'alw_[0, 100] (reactorPressure[t] < 2895)');
...
% ... Add other specification checks as needed ...
...
function logViolation(parameter, formula)
    ...
    if exist(logFileName, 'file')
        doc = xmlread(logFileName); % Read existing XML file
    else
        doc = com.mathworks.xml.XMLUtils.createDocument('Violations'); % Create new XML document
    end
    
    rootNode = doc.getDocumentElement;
    
    violationNode = doc.createElement('Violation'); % Use 'doc' instead of 'docNode'
    rootNode.appendChild(violationNode);
    
    paramNode = doc.createElement('Parameter'); % Use 'doc' instead of 'docNode'
    paramTextNode = doc.createTextNode(parameter);
    paramNode.appendChild(paramTextNode);
    violationNode.appendChild(paramNode);
    
    formulaNode = doc.createElement('Formula'); % Use 'doc' instead of 'docNode'
    formulaTextNode = doc.createTextNode(formula);
    formulaNode.appendChild(formulaTextNode);
    violationNode.appendChild(formulaNode);
    
    xmlwrite(logFileName, doc); % Write changes back to the XML file
end
...
function logFileName = getLogFileName()
    ...
    logFileName = sprintf('violation_log_%s.xml', runTimestamp);
end
